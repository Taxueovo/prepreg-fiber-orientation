...
def main() -> None:
    p = argparse.ArgumentParser(description="E02 factorial component ablation")
    ...
    p.add_argument("--weights", type=Path, required=True, help="Frozen DINOv3 checkpoint")
    ...
    (a.output_dir / "manifest.json").write_text(json.dumps({"experiment":"E02_factorial", "seeds":seeds, "configs":configs,
        "freeze_rule":"Select FFT radii and loss weights on validation data only; all eight configurations share every other training condition."}, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    for seed in seeds:
        ...
        print(f"Completed {name}, seed={seed}: {run}")
